...
function jiraProxyUrl(path) {
  return `${window.location.origin}/jira-proxy/${path}`;
}
    ...
    <div style="padding:14px 18px;background:rgba(0,229,255,0.05);border:1px solid rgba(0,229,255,0.2);border-radius:10px;">
      ...
      <div style="border-top:1px solid var(--border);padding-top:12px;">
        <div style="font-size:12px;color:var(--text-muted);line-height:1.6;">
          Polling every 60s for source team responses. New comments will appear as notifications.
        </div>
      </div>
    </div>`;
}
...
function _agentNotAvailable() { showToast('AI Agent not available in this version.', 'warn'); }
