...
# Generate 50 unique codes (reduced from 500)
codes = set()
while len(codes) < 50:
    codes.add(gen_code())
codes = list(codes)

deleted_codes = set(random.sample(codes, 4))  # Reduced from 20
changed_codes = set(random.sample([c for c in codes if c not in deleted_codes], 8))  # Reduced from 40

new_codes = set()
while len(new_codes) < 4:  # Reduced from 20
    c = gen_code()
...
sys_codes = list({str(random.randint(10000, 99999)) for _ in range(30)})[:15]  # Reduced from 150/100
...
used_keys  = set()
sscrcep_prev = []
while len(sscrcep_prev) < 80:  # Reduced from 800
...
n       = len(sscrcep_prev)
del_idx = set(random.sample(range(n), 6))  # Reduced from 30
chg_idx = set(random.sample([i for i in range(n) if i not in del_idx], 10))  # Reduced from 50

new_maps = []
while len(new_maps) < 8:  # Reduced from 40
    coe  = random.choice(sys_codes)
    coz  = coe
    sscd = random.choice(SUBSYS_CODES)
    dlcd = random.choice(DEAL_CODES)
